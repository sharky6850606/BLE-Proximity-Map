...
const SIGNAL_ALERT_THRESHOLD_DBM = -95;      // Very weak signal threshold

// Smart alert state
let deviceStatus = {};          // ident -> 'online' | 'offline'
let beaconDistanceState = {};   // key (device::beacon) -> 'safe' | 'far'
let beaconSignalState = {};     // key (device::beacon) -> 'ok' | 'low'
...
async function fetchAndUpdateMapData() {
  try {
    ...
    const STATUS_INTERVAL_MS = 30 * 60 * 1000; // 30 minutes between "still in/out" status updates per beacon
    ...
    aggBeacons.forEach(b => {
      ...
      // Distance alert: beacon has moved too far away from its device
      const prevDistState = beaconDistanceState[key] || 'safe';
      const nowDistState =
        (typeof dist === 'number' && !Number.isNaN(dist) && dist > DISTANCE_ALERT_THRESHOLD_M)
          ? 'far'
          : 'safe';
      if (prevDistState !== nowDistState) {
        beaconDistanceState[key] = nowDistState;
        if (nowDistState === 'far') {
          addNotification('distance', b.name || b.id, b.last_seen, dist);
        }
      }

      // Signal quality alert: RSSI very weak
      const rssi = b.rssi;
      const prevSignal = beaconSignalState[key] || 'ok';
      const nowSignal =
        (typeof rssi === 'number' && !Number.isNaN(rssi) && rssi <= SIGNAL_ALERT_THRESHOLD_DBM)
          ? 'low'
          : 'ok';
      if (prevSignal !== nowSignal) {
        beaconSignalState[key] = nowSignal;
        if (nowSignal === 'low') {
          addNotification('signal', b.name || b.id, b.last_seen, dist);
        }
      }
    });
