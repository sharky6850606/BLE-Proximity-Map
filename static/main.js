...
async function fetchAndUpdateMapData() {
  try {
    ...
    aggBeacons.forEach(b => {
      ...
      // Basic in/out movement notifications
      if (prev !== nowState) {
      ...
      // Distance alert: beacon further than configured threshold
      const prevFar = beaconDistanceState[key] || 'safe';
      const nowFar = dist != null && dist > DISTANCE_ALERT_THRESHOLD_METERS ? 'far' : 'safe';
      if (prevFar !== nowFar) {
        beaconDistanceState[key] = nowFar;
        if (nowFar === 'far') {
